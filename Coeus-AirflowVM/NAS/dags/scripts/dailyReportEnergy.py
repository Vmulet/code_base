...
def calculatePowerBattery(dataCurrent, dataVoltage):
    ...
    dataCurrent.iloc[:, -1] = pd.to_numeric(dataCurrent.iloc[:, -1], errors="coerce")
    dataVoltage.iloc[:, -1] = pd.to_numeric(dataVoltage.iloc[:, -1], errors="coerce")
    resultAveragePower = (dataCurrent.iloc[:, -1] * dataVoltage.iloc[:, -1]).mean()
...
def getSpotterData(report_date):
    ...
    for data in responseData["data"]["waves"]:
        timestamps.append(
            datetime.strptime(data["timestamp"], "%Y-%m-%dT%H:%M:%S.%fZ").strftime("%X")
        )
        significantWaveHeight.append(data["significantWaveHeight"])
    spotterData = {
        "acquisition_time": timestamps,
...
def parquets_to_dailyReport(file_date, parquet_files, pdf_file):
    ...
    with PdfPages(pdf_file) as pdf:
        ...
        add_text_to_fig(
            "Energy change battery Total : {}Wh ( battery 2 : {}Wh and battery 3 : {}Wh)".format(
                int(power2 + power3), int(power2), int(power3)
            ),
            fig,
            14,
            position=413,
        ...
        multi_plot(
            ...
            position,
            "Pack SOC(%)",
            ["Pack 2 10-min avg", "Pack 3 10-min avg"],
            -1,
        )
        position = 312
        multi_plot(
            [dataCurrent2, dataCurrent3],
            fig,
            position,
            "Pack Current (A)",
            ["Pack 2 10-min avg", "Pack 3 10-min avg"],
        ...
        multi_plot(
            [
                filterAveragePerPeriod(data, 6, [-4]),
                filterAveragePerPeriod(data, 600, [-4]),
            ],
            fig,
            position,
            "Anemometer wind speed (m/s)",
            ["6 seconds average", "10 minutes average"],
            -1,
        )
        save_figure(fig, pdf)
        ...
        dataPack1averaged = filterAveragePerPeriod(dataPack1, period, [5, 6, 7, 15, 17])
        dataPack2averaged = filterAveragePerPeriod(dataPack2, period, [5, 6, 7, 15, 17])
        dataPack3averaged = filterAveragePerPeriod(dataPack3, period, [5, 6, 7, 15, 17])

        fig = new_pdf_page()
        position = 321
        multi_plot(
            [dataPack1averaged, dataPack2averaged, dataPack3averaged],
            fig,
            position,
            "Solar panel input voltage(v)",
            ["Pack 1 10-min avg", "Pack 2 10-min avg", "Pack 3 10-min avg"],
            -5,
        )
        position = 322
        multi_plot(
        ...
        multi_plot(
            ...
            fig,
            position,
            "Solar panel battery temperature l(°C)",
            ["Pack 1 10-min avg", "Pack 2 10-min avg", "Pack 3 10-min avg"],
            -2,
        )
        save_figure(fig, pdf)
        ...
        dataBottomColumn = filterAveragePerPeriod(dataBottomColumn, period, [2, 3])

        fig = new_pdf_page()
        position = 211
        multi_plot(
            [dataTopColumn, dataBottomColumn],
            fig,
            position,
            "Temperature(°C)",
            ["Top Column 10-min avg", "Bottom Column 10-min avg"],
            -2,
        )
        position = 212
        multi_plot(
        ...
        multi_plot(
            [
                filter_data(data)[["acquisition_time", data.columns[2]]],
                filterAveragePerPeriod(data, 600, [2]),
            ],
            fig,
            position,
            "Pitch(°)",
            ["Data every 30 seconds", "10 minutes average"],
            -1,
        )
        position = 212
        multi_plot(
            [
                filter_data(data)[["acquisition_time", data.columns[4]]],
                filterAveragePerPeriod(data, 600, [4]),
            ],
            fig,
            position,
        ...
        fig = new_pdf_page()
        multi_plot(
            [getSpotterData(file_date)],
            fig,
            111,
            "significantWaveHeight (m)",
            ["raw significantWaveHeight data from spotter"],
            -1,
        )
        save_figure(fig, pdf)
    next_file.counter = 0
...
def plot_gps(data, fig, position, ylabel, file_date):
    ...
    long250 = (
        longitude_ref
        + Y250 / Earth_radius / np.cos(latitude_ref * np.pi / 180) * 180 / np.pi
    )
